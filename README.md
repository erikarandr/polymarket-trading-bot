...
**A high-performance, type-safe Polymarket trading bot and arbitrage engine - built in Node.js + TypeScript.**

Detects and executes on YES/NO mispricings, crypto Up/Down markets (BTC, ETH, SOL), and cross-venue inefficiencies in real time. A complete open-source **Polymarket arb bot** for automated prediction-market trading.
...
<table>
  <tr>
    <td align="center" width="50%">
      <b>Live Trading Dashboard</b><br><br>
      <img src="https://github.com/user-attachments/assets/48095fe0-e960-456b-b756-364907f05b3e" alt="Polymarket trading bot live dashboard showing open positions and P&L" width="100%">
    </td>
    <td align="center" width="50%">
      <b>Performance & Opportunity Feed</b><br><br>
      <img src="https://github.com/user-attachments/assets/554f3ddc-31e6-4ef2-bd6c-63f26f4b514d" alt="Polymarket arbitrage bot performance tracking and opportunity scanner" width="100%">
    </td>
  </tr>
<tr>
<td><img width="1851" height="1025" alt="image" src="https://github.com/user-attachments/assets/8a74930f-d970-4f60-a71c-4fc6951b0a6c" /></td>
<td><img width="1808" height="1028" alt="image" src="https://github.com/user-attachments/assets/c2329d99-bf53-4111-86f2-c93d11ec5e30" /></td>
</tr>
  <tr>
    <td><img width="1759" height="1017" alt="image" src="https://github.com/user-attachments/assets/0c1915f9-4bfb-4d3d-881d-5baf3d783054" /></td>
    <td><img width="1794" height="1016" alt="image" src="https://github.com/user-attachments/assets/7dbd3025-eb54-4284-80c8-ece3bb96f065" /></td>
  </tr>
  <tr>
<td><img width="1694" height="1021" alt="image" src="https://github.com/user-attachments/assets/29eed24e-2dd6-43f6-9724-b3fc95475d69" /></td>
  </tr>
</table>
...
- [Strategy Risk at a Glance](#strategy-risk-at-a-glance)
...
Prediction markets like Polymarket move fast, and pricing inefficiencies between YES/NO legs - or between Polymarket and other venues - tend to close within seconds. This **Polymarket trading bot** exists to make that detection-and-execution loop **fast, observable, and safe to operate**, instead of relying on manual monitoring or fragile scripts.
...
### 1. YES/NO Price Discrepancy (Parity Arbitrage)
...
**What actually determines the outcome:** fill timing dominates here more than in slower markets. A combined cost that looks attractive at scan time can be gone in the time it takes to route two orders, and a sharp last-second BTC/ETH/SOL move can push the "cheap" leg to fill at a materially worse price than it showed on the scanner. **Still hedged, but latency-sensitive - this strategy lives or dies on execution speed.**
...
**What actually determines the outcome:** this is the least "clean" strategy of the set - it depends on the two markets actually being *resolution-equivalent*, not just similar (subtly different wording, oracles, or deadlines can break the hedge entirely), on both venues having enough liquidity to fill at the quoted price, and on execution being fast enough on both legs that the gap hasn't already closed by the time both orders land. Venue-specific fees, withdrawal/settlement timing, and contract-wording differences are real risks specific to this strategy that don't apply to the single-venue ones above. **Treat equivalence checks as the highest-priority validation step before enabling this strategy live.**
...
**What actually determines the outcome:** this carries meaningfully different risk than the other three - there's no "$1.00 pair" backstop if the direction is wrong. Treat any custom signal you plug in here as unproven until you've validated it in dry-run mode against real order flow. **The only strategy in the set with unhedged market risk - size accordingly.**
...
## Strategy Risk at a Glance

| Strategy | Hedged? | Primary risk | Complexity |
|---|---|---|---|
| YES/NO Parity Arb | Yes | Execution speed | Low |
| Up/Down Arb (5-min) | Yes | Execution speed, higher competition | Medium |
| Cross-Platform Arb | Yes (if equivalent) | Market-equivalence risk | High |
| Statistical/Momentum | No | Directional market risk | Medium–High |
| Merge/Redeem | N/A | None (capital efficiency only) | Low |
...
**Is this a Polymarket arbitrage bot or a directional trading bot?**
Both, but arbitrage-first. Four of the five strategies (#1, #2, #3, #5) are hedged around the $1 YES/NO parity relationship. Only the momentum strategy (#4) takes unhedged directional positions, and it's opt-in rather than enabled by default.

**Does this Polymarket bot work with crypto Up/Down markets (BTC, ETH, SOL)?**
Yes - strategy #2 is purpose-built for the 5-minute BTC/ETH/SOL Up/Down markets, with timing and price-band logic tuned for how fast those specific order books move.

**Is Polymarket bot trading profitable?**
It depends entirely on execution speed, competition, fees, and configuration - see [Strategy Risk at a Glance](#strategy-risk-at-a-glance). This project makes no profitability guarantee; use dry-run mode to validate before risking capital.

**What language/stack is this Polymarket trading bot written in?**
Node.js 20+ and TypeScript, using ethers.js/viem for chain interaction and Zod for end-to-end type validation. See [Tech Stack](#tech-stack).

**Can I run this Polymarket bot without real capital?**
Yes - dry-run mode runs the full detection and signal pipeline with zero capital at risk, so you can validate strategy behavior before going live.
...
MIT License - free to use and modify with attribution. Thank you!

---

<div align="center">

**Keywords:** polymarket trading bot · polymarket arbitrage bot · polymarket arb bot · polymarket bot typescript · polymarket bot nodejs · node.js polymarket bot · prediction market trading bot · prediction market arbitrage · crypto up/down bot · polymarket YES NO arbitrage · polymarket automated trading · polymarket automation · defi trading bot · polymarket BTC ETH SOL bot

**Suggested GitHub topics:** `polymarket` `trading-bot` `arbitrage-bot` `prediction-markets` `typescript` `nodejs` `crypto-trading-bot` `defi` `polygon` `algorithmic-trading`
